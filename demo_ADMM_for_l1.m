...
%% COST FUNCTION
% x^* = argmin_x { 1/2 * || A(X) - Y ||_2^2 + lambda * ( || D_x(X) ||_1 + || D_y(X) ||_1 ) }
%
%% Augmented Lagrangian Method when s.t. Z = X
% argmin_X { 1/2 * || A(X) - Y ||_2^2 + lambda * ( || Dx ||_1 + || Dy ||_1 ) }
% s.t. Dx = D_x(X)
%      Dy = D_y(X)
%
% By augmented lagrangian method,
% L(X, Dx, Dy, Bx, By) = 1/2 * || A(X) - Y ||_2^2 + lambda * ( || Dx ||_1 + || Dy ||_1 ) + rho/2 * ( || Dx - D_x(X) - Bx ||_2^2 + || Dy - D_y(X) - By ||_2^2 )
%
...
% Dx^k+1 = argmin_Dx { lambda * ( || Dx ||_1 ) + rho/2 * ( || Dx - D_x(X^k+1) - Bx^k ||_2^2 ) }
%
% SubProb. 2-2 : argmin_Dx = L(X^k+1, Dx^k+1, Dy, Bx^k, By^k)
% Dy^k+1 = argmin_Dy { lambda * ( || Dy ||_1 ) + rho/2 * ( || Dy - D_y(X^k+1) - By^k ||_2^2 ) }
%
...
%% ALTERNATING DIRECTION METHOD OF MULTIPLIERS (ADMM) INITIALIZATION
% LAMBDA  = 1e0;
% RHO     = 1e4;
...
COST.equation   = '1/2 * || A(X) - Y ||_2^2 + lambda * ( || D_x(X) ||_1 + || D_y(X) ||_1 )';
COST.function	= @(x) 1/2 * L2(R(x) - y) + LAMBDA * (L1(Dx(x)) + L1(Dy(x)));
...
for iadmm = 1:nADMM
    ...
    Dx_      = threshold(DxX_ + Bx_, LAMBDA/RHO);
    Dy_      = threshold(DyX_ + By_, LAMBDA/RHO);
...
figure('name', 'Alternating Direction Method of Multifliers (ADMM) Method for l1 regularization');
colormap(gray(256));
...
subplot(235);   imagesc(x_admm,   wndImg);  	axis image off;     title({['recon_{admm-l1}'], ['MSE : ' num2str(mse_x_admm, '%.4e')], ['PSNR : ' num2str(psnr_x_admm, '%.4f')], ['SSIM : ' num2str(ssim_x_admm, '%.4f')]});
